...
class Evaluate:
    def __init__(self,
                 ...
                 custom_objects={'GeMPoolingLayer': GeMPoolingLayer, 'loss': tfa.losses.TripletSemiHardLoss()},
                 sample_size=4
                 ):
        """
        ...
        self.params_path = Path((Path(__file__) / ".." / "..").resolve()).joinpath(self.params_file_name).resolve()
        self.images_folder = Path((Path(__file__) / ".." / "..").resolve()).joinpath(self.images_folder_name).iterdir()
        self.images_test_folder = Path((Path(__file__) / ".." / "..").resolve()).joinpath(self.images_test_folder_name).iterdir()
    ...
    def load_index(self):
        # load annoy index
        u = AnnoyIndex(self.cnf.model.embeddingsize, self.cnf.annoy.similarity)
    ...
    def calculate_metric(self):
        ...
        with tqdm(total=total) as pbar:
            for test_class in self.images_test_folder:
                # skip if the items inside the class is less than sample size
                if len(os.listdir(test_class)) < self.sample_size:
                    continue
...
if __name__ == '__main__':
    evaluate = Evaluate()
    evaluate.save_index(index_name=evaluate.cnf.annoy.index_name)
